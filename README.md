...
- `README.md` 是唯一主文档
- 不再额外维护 `QuickRead.md` 和 `MANIFEST.md`
- 这样可以避免公开仓库里的步骤说明发生漂移
...
1. 带 `Ego-Planner` 的任务线
   - 主启动入口：
     - `start_sim_agent.sh`
     - `start_sim.sh`
   - planner trace / A-B 验证入口：
     - `run_planner_trace.sh`
     - `planner_trace_runner.py`
2. 固定轨迹动态性能线
   - 最小栈启动入口：
...
- `advanced_param_px4.xml`

这样 planner 的核心地图 / 碰撞 / 优化参数也进入版本管理，不再继续隐式依赖旧基线里的未公开副本。
...
- `run_planner_trace.sh`
- `planner_trace_runner.py`
- `src/ego-planner/src/planner/plan_manage/launch/advanced_param_px4.xml`
...
cp $PATCH_ROOT/catkin_ws/run_planner_trace.sh $WS/run_planner_trace.sh
cp $PATCH_ROOT/catkin_ws/planner_trace_runner.py $WS/planner_trace_runner.py

cp $PATCH_ROOT/catkin_ws/src/ego-planner/src/planner/plan_manage/launch/advanced_param_px4.xml \
  $WS/src/ego-planner/src/planner/plan_manage/launch/advanced_param_px4.xml
...
当前这套 `tracking` 脚本已经包含两条默认的可复现保护：

- `start_tracking_stack.sh` 和 `run_tracking_suite.sh` 在自己拉起栈时会默认重置持久化 SITL 参数
- 如确实需要保留当前 SITL 参数，可显式传：
  - `--no-reset-params`
- `tracking_test_runner.py` 会在 suite 结束时自动恢复实验前的 PX4 参数快照
...
perl -0pi -e 's/\r\n/\n/g' \
  ...
  $WS/run_planner_trace.sh \
  $WS/planner_trace_runner.py \
  $WS/src/ego-planner/src/planner/plan_manage/launch/advanced_param_px4.xml \
  $WS/src/ego-planner/src/planner/plan_manage/launch/px4_single.launch \
  $WS/src/ego-planner/src/planner/plan_manage/launch/default.rviz
```
...
当前 render-enabled 路径里，真实原始点云 publisher 现在优先看：
...
- `start_sim.sh` 和 `start_sim_agent.sh` 现在会把
  - `/cloud_corrected`
  - `/planning/pos_cmd`
  - `/waypoint_generator/waypoints`
  当成真正的就绪门槛
- 如果这些 topic 没起来，脚本会直接失败退出
  - 不再把“planner 没起来但系统还在悬停”伪装成一次成功启动

当前两个主启动脚本还会默认注入一组已经验证过的运行参数：

- `/obstacle_generator/seed=250`
- `/cloud_fixer_node/point_stride=4`
- `/cloud_fixer_node/max_publish_hz=0`
- `/px4_bridge_machine/enable_smoothing=true`
- `/px4_bridge_machine/smooth_alpha=0.6`
- `/px4_bridge_machine/zero_accel_when_shaping=false`

如果你需要临时覆盖这些值，可以直接带环境变量启动，例如：

```bash
BRIDGE_ENABLE_SMOOTHING=false ./start_sim_agent.sh
CLOUD_POINT_STRIDE=2 ./start_sim.sh
PLANNER_MAX_VEL=1.4 PLANNER_MAX_ACC=1.8 ./start_sim_agent.sh
PLANNER_HORIZON=10 PLANNER_LAMBDA_COLLISION=12.0 PLANNER_DIST0=0.45 ./start_sim.sh
```

当前 launcher 也支持这几个 planner 侧环境变量覆盖，而不需要手改
`advanced_param_px4.xml`：

- `PLANNER_MAX_VEL`
- `PLANNER_MAX_ACC`
- `PLANNER_HORIZON`
- `PLANNER_LAMBDA_COLLISION`
- `PLANNER_DIST0`

planner trace 验证脚本也复用同一套 planner 环境变量覆盖：

```bash
PLANNER_MAX_VEL=1.3 PLANNER_MAX_ACC=1.6 ./run_planner_trace.sh --label probe_v13_a16
PLANNER_HORIZON=10 ./run_planner_trace.sh --label probe_h10
```
...
## 第四层验证：planner trace 验证

如果你要验证 planner 参数覆写和 trace 采集链路，当前推荐直接跑：

```bash
cd $WS
./stop_sim.sh
./run_planner_trace.sh --label planner_base --duration 25
```
...
- 采集：
  ...
  - `/cloud_corrected`
- 输出到：
  - `~/catkin_ws/records/planner_eval/<timestamp>_<label>/`
...
当前 `tracking/` 目录还有两个新的可复现性约束：

1. `start_tracking_stack.sh` 和 `run_tracking_suite.sh` 默认都会要求干净参数起点
   - 如确实需要保留当前 SITL 参数，可显式传：
   - `--no-reset-params`
2. `tracking_test_runner.py` 现在会在测试结束后自动恢复 `params_before.json`
   - 即使某个 profile 中途失败，也会先尝试回滚被改过的 PX4 参数
...
- 带 `Ego-Planner`
- 带任务 waypoint
- 排查 perception / bridge / planner 主链
