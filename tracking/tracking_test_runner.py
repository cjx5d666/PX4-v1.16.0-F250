...
LEVEL_CONFIGS: Dict[str, Dict[str, float]] = {
    ...
    "l2": {
        ...
        "figure8_amp_x": 0.80,
        "figure8_amp_y": 0.50,
        "figure8_freq_hz": 0.09,
        "z_sine_amp": 0.20,
        "z_sine_freq_hz": 0.045,
        "diag_xy_step_amp": 0.50,
        "diag_xy_ramp_amp": 0.80,
    },
}
...
class TrackingTestRunner:
    ...
    def snapshot_params(self, target_path: str, extra_names: Optional[List[str]] = None) -> Dict[str, float]:
        self._pull_params()
        param_names = list(KEY_PARAMS)
        if extra_names:
            for name in extra_names:
                if name not in param_names:
                    param_names.append(name)
        values = {name: self._get_param(name) for name in param_names}
    ...
    def restore_params(self, values: Dict[str, float], target_path: str) -> Dict[str, Dict[str, float]]:
        results = {}
        for name, expected in values.items():
            after = self._set_param(name, expected)
            results[name] = {"requested": expected, "after": after}
        with open(target_path, "w", encoding="utf-8") as handle:
            json.dump(results, handle, indent=2, sort_keys=True)
        return results
    ...
    def run_suite(self, suite_name: str, level_name: str, profiles: List[ProfileDefinition], param_updates: Dict[str, float]) -> None:
        ...
        tracked_param_names = list(param_updates.keys())
        params_before = self.snapshot_params(
            os.path.join(self.output_dir, "params_before.json"),
            extra_names=tracked_param_names,
        )
        ...
        try:
            ...
            self.snapshot_params(
                os.path.join(self.output_dir, "params_effective.json"),
                extra_names=tracked_param_names,
            )
            ...
            try:
                self.restore_params(params_before, os.path.join(self.output_dir, "params_restored.json"))
            except Exception as exc:
                restore_error = exc

        if restore_error is not None and run_error is not None:
            raise RuntimeError(
                f"Tracking suite failed ({run_error}) and PX4 param restore also failed ({restore_error})"
            )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--suite", default="default", choices=["default", "smoke", "z_only", "diag_xy", "showcase"])
